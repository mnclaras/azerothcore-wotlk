...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
    {
        ...
        case 1: // Create new Arenateam
        {
            if (sWorld->getIntConfig(CONFIG_SOLO_3V3_MIN_LEVEL) <= player->getLevel())
            {
                if (player->GetMoney() >= sWorld->getIntConfig(CONFIG_SOLO_3V3_COSTS) && CreateArenateam(player, creature))
        ...
        case 2: // 3v3 Join Queue Arena (rated)
        {
                ...
                (sWorld->getBoolConfig(CONFIG_SOLO_3V3_CAST_DESERTER_ON_AFK) || sWorld->getBoolConfig(CONFIG_SOLO_3V3_CAST_DESERTER_ON_LEAVE)))
            {
                ...
                if (ArenaCheckFullEquipAndTalents(player)
                    && JoinQueueArena(player, creature, true, ARENA_TYPE_3v3_SOLO) == false)
    ...
    struct npc_solo3v3AI : public ScriptedAI
    {
        npc_solo3v3AI(Creature* creature) : ScriptedAI(creature) { }
    };

    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_solo3v3AI(creature);
    }
