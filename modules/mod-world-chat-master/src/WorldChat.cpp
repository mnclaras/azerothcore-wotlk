...
public:
    ...
    static bool HandleWorldChatCommand(ChatHandler * pChat, const char * msg)
    {
        ...
        if (!sConfigMgr->GetBoolDefault("World_Chat.Enable", true)) {
			ChatHandler(pChat->GetSession()).PSendSysMessage("[WC] %sEste chat se ha deshabilitado. Ahora el canal del mundo es el global. Puedes unirte escribiendo /join global|r", WORLD_CHAT_RED.c_str());
            return true;
        }
    ...
    static bool HandleWorldChatOnCommand(ChatHandler* handler, const char* /*msg*/)
    {
        ...
        if (!sConfigMgr->GetBoolDefault("World_Chat.Enable", true)) {
			ChatHandler(player->GetSession()).PSendSysMessage("[WC] %sEste chat se ha deshabilitado. Ahora el canal del mundo es el global. Puedes unirte escribiendo /join global|r", WORLD_CHAT_RED.c_str());
            return true;
        }
